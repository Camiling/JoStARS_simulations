...
df.time.K2 = data.frame(time=c(time.res.joint.K2$times.jointGHS, time.res.joint.K2$times.JGL, time.res.joint.K2$times.SSJGL, time.res.joint.K2$times.GGL,time.res.joint.K2$times.jostars), p=rep(p,5),
                        method=c(rep('jointGHS',n.p),rep('FGL',n.p),rep('SSJGL',n.p),rep('GGL',n.p),rep('JoStARS',n.p)))

df.time.K3 = data.frame(time=c(time.res.joint.K3$times.jointGHS, time.res.joint.K3$times.JGL, time.res.joint.K3$times.SSJGL, time.res.joint.K3$times.GGL,time.res.joint.K3$times.jostars), p=rep(p,5),
                        method=c(rep('jointGHS',n.p),rep('FGL',n.p),rep('SSJGL',n.p),rep('GGL',n.p),rep('JoStARS',n.p)))

df.time.K4 = data.frame(time=c(time.res.joint.K4$times.jointGHS, time.res.joint.K4$times.JGL, time.res.joint.K4$times.SSJGL, time.res.joint.K4$times.GGL,time.res.joint.K4$times.jostars), p=rep(p,5),
                        method=c(rep('jointGHS',n.p),rep('FGL',n.p),rep('SSJGL',n.p),rep('GGL',n.p),rep('JoStARS',n.p)))
...
ggplot2::ggplot(df.time.no.jointGHS, aes(y=time, x=p, group=method))+ labs(title=" ")+theme_bw()+theme(plot.title = element_text(hjust = 0.5),text = element_text(size = 15))+
  geom_line(aes(colour=method, linetype=method), linewidth=1.2)+ scale_color_manual(values = c('olivedrab3','dodgerblue','darkorange','maroon2'))+
  labs(y="CPU time (s)")+ scale_x_continuous(breaks = p)+scale_y_continuous(trans='log10',breaks = c(0,5, 50,500,5000,50000))+facet_wrap(~K)
dev.off()

df.time.no.onlyJGL = df.time %>% filter(method %in% c('FGL','JoStARS'))
...
ggplot2::ggplot(df.time.no.onlyJGL, aes(y=time, x=p, group=method))+ labs(title=" ")+theme_bw()+theme(plot.title = element_text(hjust = 0.5),text = element_text(size = 15))+
  geom_line(aes(colour=method, linetype=method), linewidth=1.2)+ scale_color_manual(values = c('olivedrab3','darkorange'))+
  labs(y="CPU time (s)")+ scale_x_continuous(breaks = p)+scale_y_continuous(trans='log10',breaks = c(0,5, 50,500,5000,50000))+facet_wrap(~K)
dev.off()


df.time.no.onlyJGLSSJGL = df.time %>% filter(method %in% c('FGL','JoStARS','SSJGL'))
...
ggplot2::ggplot(df.time.no.onlyJGLSSJGL, aes(y=time, x=p, group=method))+ labs(title=" ")+theme_bw()+theme(plot.title = element_text(hjust = 0.5),text = element_text(size = 15))+
  geom_line(aes(colour=method, linetype=method), linewidth=1.2)+ scale_color_manual(values = c('olivedrab3','darkorange','maroon2'))+
  labs(y="CPU time (s)")+ scale_x_continuous(breaks = p)+scale_y_continuous(trans='log10',breaks = c(0,5, 50,500,5000,50000))+facet_wrap(~K)
dev.off()
...
ggplot2::ggplot(df.time, aes(y=time, x=p, group=method))+ labs(title=" ")+theme_bw()+theme(plot.title = element_text(hjust = 0.5),text = element_text(size = 15))+
  geom_line(aes(colour=method, linetype=method), size=1.2)+ scale_color_manual(values = c('olivedrab3','dodgerblue','blueviolet','darkorange','maroon2'))+
  labs(y="CPU time (s)")+ scale_x_continuous(breaks = p)+scale_y_continuous(trans='log10',breaks = c(0,5, 50,500,5000,50000))+facet_wrap(~K)
dev.off()
