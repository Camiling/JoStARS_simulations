    ...
    if(show.interval){
      ...
      for (i in 1:length(obj.list)){
        ...
        if(include.glasso){
          cat(' & Glasso ')
          if(show.lambda){
            cat('&', round(mean(obj$lambda.glasso),3), '& -')
          }
          for(k in 1:K){
        ...
        if(include.JGL){
          cat('  & FGL ')  
          if(show.lambda){
            cat('&', round(mean(obj$lambda1.jgl),3), '&',round(mean(obj$lambda2.jgl),3))
          }
          for(k in 1:K){
        ...
        if(include.GGL){
          cat('  & GGL ')  
          if(show.lambda){
            cat('&', round(mean(obj$lambda1.ggl),3), '&',round(mean(obj$lambda2.ggl),3))
          }
          for(k in 1:K){
            cat(' && ',round(obj$mean.opt.sparsities.ggl[k],3), '[',paste(round(quantile(obj$opt.sparsities.ggl[,k],probs=c(.025,.975)),3),collapse=','),'] &',
                round(obj$mean.precisions.ggl[k],2),'[',paste(round(quantile(obj$precisions.ggl[,k],probs=c(.025,.975)),3),collapse=','),'] &',
                round(obj$mean.recalls.ggl[k],2),'[',paste(round(quantile(obj$recalls.ggl[,k],probs=c(.025,.975)),3),collapse=','),']')
            if(show.specificity)cat('&',round(obj$mean.specificities.ggl[k],2),'[',paste(round(quantile(obj$specificities.ggl[,k],probs=c(.025,.975)),3),collapse=','),']') 
            if(show.distance) cat(' & ',round(obj$mean.matrix.distances.ggl[k],3))
          }  
          cat(' \\\\ \n')
        }
        if(include.SSJGL){
        ...
        if(include.jointGHS){
          ...
          for(k in 1:K){
            cat(' && ',round(obj$mean.opt.sparsities.jointghs[k],3), '[',paste(round(quantile(obj$opt.sparsities.jointghs[,k],probs=c(.025,.975)),3),collapse=','),'] &',
                round(obj$mean.precisions.jointghs[k],2),'[',paste(round(quantile(obj$precisions.jointghs[,k],probs=c(.025,.975)),3),collapse=','),'] &',
                round(obj$mean.recalls.jointghs[k],2),'[',paste(round(quantile(obj$recalls.jointghs[,k],probs=c(.025,.975)),3),collapse=','),']')
            if(show.specificity)cat('&',round(obj$mean.specificities.jointghs[k],2), '[',paste(round(quantile(obj$specificities.jointghs[,k],probs=c(.025,.975)),3),collapse=','),']') 
            if(show.distance) cat(' & ',round(obj$mean.matrix.distances.jointghs[k],3))
          }  
          cat(' \\\\ \n \\hline \n')         
        }
        if(include.stabJGL){
      ...
      for (i in 1:length(obj.list)){
        ...
        if(include.GGL){
          ...
          for(k in 1:K){
            cat(' && ',round(obj$mean.opt.sparsities.ggl[k],3), '& ',
                round(obj$mean.precisions.ggl[k],2),'& ',
                round(obj$mean.recalls.ggl[k],2))
            if(show.specificity)cat('&',round(obj$mean.specificities.ggl[k],2)) 
            if(show.distance) cat(' & ',round(obj$mean.matrix.distances.ggl[k],3))
          }  
          cat(' \\\\ \n')
        }
        ...
        if(include.jointGHS){
          ...
          for(k in 1:K){
            cat(' && ',round(obj$mean.opt.sparsities.jointghs[k],3), '& ',
                round(obj$mean.precisions.jointghs[k],2),' & ',
                round(obj$mean.recalls.jointghs[k],2))
            if(show.specificity)cat('&',round(obj$mean.specificities.jointghs[k],2)) 
            if(show.distance) cat(' & ',round(obj$mean.matrix.distances.jointghs[k],3))
          } 
          cat(' \\\\ \n')
        }
...
print_results_stabJGL_show_SD = function(obj.list,fracs.mutated, include.stabJGL = TRUE, include.jointGHS=TRUE, include.glasso=TRUE, include.SSJGL=TRUE, include.JGL=TRUE,include.GGL=TRUE,show.distance=F,show.specificity=F){
  ...
  for (i in 1:length(obj.list)){
    ...
    if(include.GGL){
      cat('  & GGL ')  
      for(k in 1:K){
        cat(' && ',round(obj$mean.opt.sparsities.ggl[k],3), '(',round(sd(obj$opt.sparsities.ggl[,k]),3),')',' & ',
            round(obj$mean.precisions.ggl[k],2),'(',round(sd(obj$precisions.ggl[,k]),2),')',' & ',
            round(obj$mean.recalls.ggl[k],2),'(',round(sd(obj$recalls.ggl[,k]),2),')')
        if(show.specificity)cat('&',round(obj$mean.specificities.ggl[k],2), '(',round(sd(obj$specificities.ggl[,k]),2),')') 
        if(show.distance) cat(' & ',round(obj$mean.matrix.distances.ggl[k],3))
      }  
      cat(' \\\\ \n')
    }
    ...
    if(include.jointGHS){
      cat('  & jointGHS ')
      for(k in 1:K){
        cat(' && ',round(obj$mean.opt.sparsities.jointghs[k],3), '(',round(sd(obj$opt.sparsities.jointghs[,k]),3),')',' & ',
            round(obj$mean.precisions.jointghs[k],2),'(',round(sd(obj$precisions.jointghs[,k]),2),')',' & ',
            round(obj$mean.recalls.jointghs[k],2),'(',round(sd(obj$recalls.jointghs[,k]),2),')')
        if(show.specificity)cat('&',round(obj$mean.specificities.jointghs[k],2), '(',round(sd(obj$specificities.jointghs[,k]),2),')') 
        if(show.distance) cat(' & ',round(obj$mean.matrix.distances.jointghs[k],3))
      }  
      cat(' \\\\ \n')
    }
...
print_results_stabJGL_collapsed = function(obj.list,fracs.mutated,show.lambda=FALSE,include.stabJGL = TRUE, include.jointGHS=TRUE, include.glasso=TRUE, include.SSJGL=TRUE, include.JGL=TRUE,include.GGL=TRUE,show.distance=F,show.specificity=F,show.interval=F, show.sd =T){
  ...
  for (i in 1:length(obj.list)){
    ...
    if(include.GGL){
      cat('  & GGL ')  
      if(show.sd){
        sep1 = c('(',round(mean(apply(obj$opt.sparsities.ggl, 2, sd)),3),') & ')
        sep2 = c('(',round(mean(apply(obj$precisions.ggl, 2, sd)),2),') & ')
        sep3 = c('(',round(mean(apply(obj$recalls.ggl, 2, sd)),2),')')
        if (show.specificity) {
          sep4 = c('(',round(mean(apply(obj$specificities.ggl, 2, sd)),2),')')
        }
      }
      else{
        sep1 = sep2 = ' & '
      ...
      cat(' && ',round(mean(colMeans(obj$opt.sparsities.ggl)),3), sep1,
          round(mean(colMeans(obj$precisions.ggl)),2), sep2,
          round(mean(colMeans(obj$recalls.ggl)),2), sep3)
      if(show.specificity)cat('&',round(mean(colMeans(obj$specificities.ggl)),2), sep4) 
      if(show.distance) cat(' & ',round(mean(colMeans(obj$matrix.distances.ggl)),3))
      cat(' \\\\ \n')
    }
    if(include.SSJGL){
      cat('  & SSJGL ')  
    ...
    if(include.jointGHS){
      cat('  & jointGHS ')
      if(show.sd){
        sep1 = c('(',round(mean(apply(obj$opt.sparsities.jointghs, 2, sd)),3),') & ')
        sep2 = c('(',round(mean(apply(obj$precisions.jointghs, 2, sd)),2),') & ')
        sep3 = c('(',round(mean(apply(obj$recalls.jointghs, 2, sd)),2),')')
        if (show.specificity) {
          sep4 = c('(',round(mean(apply(obj$specificities.jointghs, 2, sd)),2),')')
        }
      }
      else{
        sep1 = sep2 = ' & '
      ...
      cat(' && ',round(mean(colMeans(obj$opt.sparsities.jointghs)),3),sep1,
          round(mean(colMeans(obj$precisions.jointghs)),2),sep2,
          round(mean(colMeans(obj$recalls.jointghs)),2), sep3)
      if(show.specificity)cat('&',round(mean(colMeans(obj$specificities.jointghs)),2), sep4) 
...
print_results_stabJGL_show_SD_lambda = function(obj.list,fracs.mutated, include.stabJGL = TRUE, include.jointGHS=TRUE, include.glasso=TRUE, include.SSJGL=TRUE, include.JGL=TRUE,include.GGL=TRUE,show.distance=F,show.specificity=F){
  ...
  for (i in 1:length(obj.list)){
    ...
    if(include.glasso){
      cat(' & Glasso ')
      cat('& ', round(mean(obj$mean.lambda.glasso),3),' & - ')
      for(k in 1:K){
        cat(paste0(' && ',round(obj$mean.opt.sparsities.glasso[k],3), ' (',round(sd(obj$opt.sparsities.glasso[,k]),3),') ',' & ',
            round(obj$mean.precisions.glasso[k],2),' (',round(sd(obj$precisions.glasso[,k]),2),') ',' & ',
            round(obj$mean.recalls.glasso[k],2), ' (',round(sd(obj$recalls.glasso[,k]),2),') '))
        if(show.specificity)cat(' & ',round(obj$mean.specificities.glasso[k],2), ' (',round(sd(obj$specificities.glasso[,k]),2),') ')
        if(show.distance) cat(' & ',round(obj$mean.matrix.distances.glasso[k],3))
    ...
    if(include.JGL){
      cat('  & FGL ')
      cat(' & ', round(obj$mean.lambda1.jgl,3),' & ',round(obj$mean.lambda2.jgl,3))
      for(k in 1:K){
        cat(paste0(' && ',round(obj$mean.opt.sparsities.jgl[k],3), ' (',round(sd(obj$opt.sparsities.jgl[,k]),3),') ',' & ',
            round(obj$mean.precisions.jgl[k],2),' (',round(sd(obj$precisions.jgl[,k]),2),') ',' & ',
            round(obj$mean.recalls.jgl[k],2),' (',round(sd(obj$recalls.jgl[,k]),2),') '))
        if(show.specificity)cat('&',round(obj$mean.specificities.jgl[k],2), '(',round(sd(obj$specificities.jgl[,k]),2),')') 
        if(show.distance) cat(' & ',round(obj$mean.matrix.distances.jgl[k],3))
    ...
    if(include.GGL){
      cat('  & GGL ')  
      cat(' & ', round(obj$mean.lambda1.ggl,3),' & ',round(obj$mean.lambda2.ggl,3))
      for(k in 1:K){
        cat(paste0(' && ',round(obj$mean.opt.sparsities.ggl[k],3), ' (',round(sd(obj$opt.sparsities.ggl[,k]),3),') ',' & ',
            round(obj$mean.precisions.ggl[k],2),' (',round(sd(obj$precisions.ggl[,k]),2),') ',' & ',
            round(obj$mean.recalls.ggl[k],2),' (',round(sd(obj$recalls.ggl[,k]),2),') '))
        if(show.specificity)cat('&',round(obj$mean.specificities.ggl[k],2), '(',round(sd(obj$specificities.ggl[,k]),2),')') 
        if(show.distance) cat(' & ',round(obj$mean.matrix.distances.ggl[k],3))
    ...
    if(include.SSJGL){
      ...
      cat('& - & - ')
      for(k in 1:K){
        cat(paste0(' && ',round(obj$mean.opt.sparsities.ssjgl[k],3), ' (',round(sd(obj$opt.sparsities.ssjgl[,k]),3),') ',' & ',
            round(obj$mean.precisions.ssjgl[k],2),' (',round(sd(obj$precisions.ssjgl[,k]),2),') ',' & ',
            round(obj$mean.recalls.ssjgl[k],2),' (',round(sd(obj$recalls.ssjgl[,k]),2),') '))
        if(show.specificity)cat('&',round(obj$mean.specificities.ssjgl[k],2), '(',round(sd(obj$specificities.ssjgl[,k]),2),')') 
        if(show.distance) cat(' & ',round(obj$mean.matrix.distances.ssjgl[k],3))
    ...
    if(include.jointGHS){
      ...
      cat('& - & - ')
      for(k in 1:K){
        cat(paste0(' && ',round(obj$mean.opt.sparsities.jointghs[k],3), ' (',round(sd(obj$opt.sparsities.jointghs[,k]),3),') ',' & ',
            round(obj$mean.precisions.jointghs[k],2),' (',round(sd(obj$precisions.jointghs[,k]),2),') ',' & ',
            round(obj$mean.recalls.jointghs[k],2),' (',round(sd(obj$recalls.jointghs[,k]),2),') '))
        if(show.specificity)cat('&',round(obj$mean.specificities.jointghs[k],2), '(',round(sd(obj$specificities.jointghs[,k]),2),')') 
        if(show.distance) cat(' & ',round(obj$mean.matrix.distances.jointghs[k],3))
